...
public interface CompressionCodec extends Closeable {
   ...
   * @param in the bytes to compress 待压缩的字节流
   * @param out the compressed bytes 压缩完成用于输出的字节流
   * @param overflow put any additional bytes here
   * @param options the options to control compression 压缩控制选项
   * @return true if the output is smaller than input
   * @throws IOException
   */
  boolean compress(ByteBuffer in, ByteBuffer out, ByteBuffer overflow, Options options) throws IOException;
