...
public class TypeDescription
  ...
  public TypeDescription addField(String field, TypeDescription fieldType) {
    ...
    if (category != Category.STRUCT) {
      throw new IllegalArgumentException("Can only add fields to struct type" + " and not " + category);
    }
  ...
  /**
   * 使用原始列向量类型创建 VectorizedRowBatch
   * Create a VectorizedRowBatch with the original ColumnVector types
   * @return a new VectorizedRowBatch
   */
  public VectorizedRowBatch createRowBatch() {
    return createRowBatch(RowBatchVersion.ORIGINAL, VectorizedRowBatch.DEFAULT_SIZE);
  }
  ...
  private int id = -1;
  private int maxId = -1;
  private TypeDescription parent;                 // root的 parent 为null
  private final Category category;                // 类型
  private final List<TypeDescription> children;   // 嵌套类型的子字段类
  private final List<String> fieldNames;          // 嵌套类型的子字段名称
