...
public class HadoopShimsFactory {
  ...
  public static synchronized HadoopShims get() {
    if (SHIMS == null) {
      ...
      int major = Integer.parseInt(versionParts[0]);  // 大版本
      int minor = Integer.parseInt(versionParts[1]);  // 小版本

      if (major < 2 || (major == 2 && minor < 7)) {   // hadoop 2.6 以下
        LOG.warn("Hadoop " + VersionInfo.getVersion() + " support is deprecated. " +
            "Please upgrade to Hadoop 2.7.3 or above.");
      }
      if (major < 2 || (major == 2 && minor < 3)) { // hadoop 2.3 以下
        SHIMS = new HadoopShimsPre2_3();
      } else if (major == 2 && minor < 6) {         // hadoop 2.6 以下
        SHIMS = createShimByName(PRE_2_6_SHIM_NAME);
      } else if (major == 2 && minor < 7) {         // hadoop 2.7 以下
