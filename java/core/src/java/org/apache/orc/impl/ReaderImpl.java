...
public class ReaderImpl implements Reader {
  ...
  private final int metadataSize;                               // 从 PostScript 中读取的 metadata 的大小
  protected final List<OrcProto.Type> types;                    // 从 Footer 中读取的 schema 的序列化形式
  private final TypeDescription schema;                         // 从 Footer 中读取的 schema
  private final List<OrcProto.UserMetadataItem> userMetadata;   // 从 Footer 中得到的 用户自定义属性
  private final List<OrcProto.ColumnStatistics> fileStats;      // 从 Footer 中得到的文件级别的 columnstatistics
  private final List<StripeInformation> stripes;                // 从 Footer 中得到的 StripeInformation
  protected final int rowIndexStride;                           // 从 Footer 中得到的 行组大小
  private final long contentLength, numberOfRows;               // 从 Footer 中得到的 body 大小，总的行数量
  ...
  protected final OrcTail tail;           // 记录当前ORC文件的 PostScript 跟 Footer 的内容
  ...
  * Constructor that let's the user specify additional options.
   * @param path ORC 的文件目录
   * @param options 读取的文件选项
   */
  public ReaderImpl(Path path, OrcFile.ReaderOptions options) throws IOException {
    ...
    if (fileMetadata != null) {
      ...
      if (orcTail == null) {
        // 从 ORC 文件中读取 Footer 区域 与 PostScript 区域
        // 并将内容填充到OrcTail中
      ...
      this.compressionKind = tail.getCompressionKind();                 // 获取当前的流压缩类型
      this.bufferSize = tail.getCompressionBufferSize();                // 获取当前的 buffer 大小
      this.metadataSize = tail.getMetadataSize();                       // 获取当前的 metadata 的大小
      this.versionList = tail.getPostScript().getVersionList();         // 获取当前的版本
      this.schema = tail.getSchema();                                   // 拿到当前的Schema
      this.rowIndexStride = tail.getFooter().getRowIndexStride();       // 获取行组大小
      this.contentLength = tail.getFooter().getContentLength();         // 获取body大小
      this.numberOfRows = tail.getFooter().getNumberOfRows();           // 获取当前的数据记录数
      this.userMetadata = tail.getFooter().getMetadataList();           // 获取用户的自定义属性
      this.fileStats = tail.getFooter().getStatisticsList();            // 拿到文件状态
      this.writerVersion = tail.getWriterVersion();
      this.stripes = tail.getStripes();                                 // 获取 strips 统计信息
  ...
  /***
   * 从文件中读取 PostScript
   * @param buffer 读取内容的Buffer
   * @param path   ORC 文件路径
   * @param psLen  PostScript 数据流长度
   * @param psOffset  PostScript 数据流开始位置
   */
  ...
  /***
   * 读取 ORC 文件尾部，读取并解析文件的 PostScript 区域与 Footer 区域
   * @param fs
   * @param path
   * @param maxFileLength
   * @return
   * @throws IOException
   */
  protected OrcTail extractFileTail(FileSystem fs,
                                    Path path,
    ...
    try {
      ...
      if (maxFileLength == Long.MAX_VALUE) {
        FileStatus fileStatus = fs.getFileStatus(path);       // 拿到文件状态
        size = fileStatus.getLen();                           // 拿到文件大小
        modificationTime = fileStatus.getModificationTime();  // 拿到文件状态修改时间
      } else {
        size = maxFileLength;
        modificationTime = -1;
      }
      ...
      int footerSize = (int) ps.getFooterLength();                 // 获取 footer 部分的内容
      int metadataSize = (int) ps.getMetadataLength();             // 获取 metadata 部分的内容
      int stripeStatSize = (int) ps.getStripeStatisticsLength();   // 获取 stripestatsize 部分的内容

      //check if extra bytes need to be read
      int tailSize = 1 + psLen + footerSize + metadataSize + stripeStatSize;  // 尾部大小定义
      ...
      try (CompressionCodec codec = OrcCodecPool.getCodec(compressionKind)) {
        ...
        OrcProto.Footer footer =
            OrcProto.Footer.parseFrom(
                InStream.createCodedInputStream
                    (InStream.create("footer", buffer, psOffset - footerSize, footerSize, compression)));
