 ...
 * 这个class处理 StripeInformation 的解析以及必要过滤和选择。
 *
 * It supports:
 * <ul>
...
public class StripePlanner {
  ...
  private final List<StreamInformation> indexStreams = new ArrayList<>();       // 用于存放 Index 类型流信息
  private final List<StreamInformation> dataStreams = new ArrayList<>();        // 用于存放数据类型的流信息
  private final OrcProto.Stream.Kind[] bloomFilterKinds;    // 指示当前的列是否支持BloomFilter
  // does each column have a null stream?
  private final boolean[] hasNull;                           // 支持当前列是否有空值
  ...
  /**
   * 解析新的 stripe 重置当前stripe的状态。
   * 填充 data 区域的 stream 信息
   * 填充 index 区域的 stream 信息
   * 填充列是否支持BloomFilter 内容
   * 填充列是否有空值内容
  ...
  /**
   * 从文件中读取 stripe
   * @param index           当前Stripe的Index内容
   * @param rowGroupInclude 对于当前stripe中的每个行组，所有行都为 null 或带有布尔值的数组。
   * @param forceDirect     是否应该创捷0拷贝
   * @param readPhase       影响读取的列，例如如果 readPhase = LEADERS 则只读取 FILTER 列所需的数据
   */
  public BufferChunkList readData(OrcIndex index,
    ...
    BufferChunkList chunks = (index == null || rowGroupInclude == null)
        ? planDataReading(readPhase) : planPartialDataReading(index, rowGroupInclude, readPhase);
      ...
      if (columnInclude[column] && encryption.getVariant(column) == variant) {
        ...
        if (kind != OrcProto.Stream.Kind.BLOOM_FILTER ||
          ...
          if (kind == OrcProto.Stream.Kind.BLOOM_FILTER_UTF8 ||
            ...
            bloomFilterKinds[column] = kind;  // 当前
          }
   ...
   * @param streamStart 当前 stripe 的起始位置
   * @param footer StripeFooter
   * @param columnInclude which columns are being read
   */
  ...
  /**
   * 读取加载INDEX区域内容， 包含 ROW_INDEX 跟 BloomFilter
  ...
  public OrcIndex readRowIndex(boolean[] sargColumns,
    ...
    System.arraycopy(bloomFilterKinds, 0, output.getBloomFilterKinds(), 0, bloomFilterKinds.length);
    ...
    for(StreamInformation stream: indexStreams) {
      ...
      if (stream.firstChunk != null) {
        CodedInputStream data = InStream.createCodedInputStream(
            InStream.create(
            "index", stream.firstChunk, stream.offset, stream.length, getStreamOptions(column, stream.kind)));
