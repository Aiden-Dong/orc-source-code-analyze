...
public abstract class StringBaseTreeWriter extends TreeWriterBase {
  ...
  private final List<OrcProto.RowIndexEntry> savedRowIndex = new ArrayList<>();
  ...
  private static Dictionary createDict(Configuration conf) {
    String dictImpl = conf.get(DICTIONARY_IMPL.getAttribute(), DICTIONARY_IMPL.getDefaultValue().toString()).toUpperCase();
  ...
  StringBaseTreeWriter(TypeDescription schema,
    ...
    directStreamOutput = context.createStream(new StreamName(id, OrcProto.Stream.Kind.DATA, encryption));
    stringOutput = context.createStream(new StreamName(id, OrcProto.Stream.Kind.DICTIONARY_DATA, encryption));
    lengthOutput = createIntegerWriter(context.createStream(new StreamName(id, OrcProto.Stream.Kind.LENGTH, encryption)), false, isDirectV2, context);
    rowOutput = createIntegerWriter(directStreamOutput, false, isDirectV2, context);
