...
public interface TreeWriter {
   ...
   * This is used as an estimate for the query optimizer.
   * @return the number of bytes
   */
  long getRawDataSize();
   ...
   * 将VectorizedRowBatch写入文件。这由顶层的WriterImplV2调用。
   *
   ...
   * This is called recursively by writeRootBatch.
   * 将列向量写入文件。
   * writeRootBatch递归调用此函数。
   *
  ...
  class Factory {
    /**
     * 为给定的类型创建一个新的tree writer，并在需要时插入加密。
     * @param schema 要构建写入器的类型
     * @param encryption the encryption status
     * @param streamFactory {@link org.apache.orc.impl.WriterImpl.StreamFactory}
     ...
     * @param streamFactory the writer context {@link org.apache.orc.impl.WriterImpl.StreamFactory}
     * @return a new tree writer
     */
    static TreeWriter createSubtree(TypeDescription schema, WriterEncryptionVariant encryption, WriterContext streamFactory) throws IOException {
      ...
      case DECIMAL:
        if (version == OrcFile.Version.UNSTABLE_PRE_2_0 && schema.getPrecision() <= TypeDescription.MAX_DECIMAL64_PRECISION) {
