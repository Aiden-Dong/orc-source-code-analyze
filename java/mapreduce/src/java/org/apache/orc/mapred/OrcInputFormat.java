...
public class OrcInputFormat<V extends WritableComparable>
  ...
  public static Reader.Options buildOptions(Configuration conf,
    ...
    Reader.Options options = reader.options()
        .range(start, length)    // 限制数据读取长度
        .useZeroCopy(OrcConf.USE_ZEROCOPY.getBoolean(conf))                      // 定义是否开启零拷贝
        .skipCorruptRecords(OrcConf.SKIP_CORRUPT_DATA.getBoolean(conf))          // 定义是否跳过数据损坏记录
    ...
    String kryoSarg = OrcConf.KRYO_SARG.getString(conf);         // 设置 SearchArgument 表达式
    ...
    if (kryoSarg != null && sargColumns != null) {
      ...
      SearchArgument sarg = new Kryo().readObject(new Input(sargBytes), SearchArgumentImpl.class);
